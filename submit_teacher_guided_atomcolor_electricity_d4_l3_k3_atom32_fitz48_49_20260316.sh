#!/usr/bin/env bash
#SBATCH --job-name=msdt_tga_elec_d4_l3
#SBATCH --output=msdt_tga_elec_d4_l3_%j.out
#SBATCH --error=msdt_tga_elec_d4_l3_%j.err
#SBATCH --time=08:00:00
#SBATCH --partition=compsci-gpu
#SBATCH --mem=200G
#SBATCH --cpus-per-task=3
#SBATCH --ntasks=1
#SBATCH --nodelist=compsci-cluster-fitz-[48-49]
...
/usr/bin/time -v .venv/bin/python run_multisplit_experiments_lightgbm.py \
  --run-name benchmark_teacher_guided_atomcolor_electricity_d4_l3_k3_atom32_20260316 \
  ...
  --test-size 0.2 \
  --parallel-trials 1 \
  --threads-per-trial "${SLURM_CPUS_PER_TASK:-3}" \
  --cpu-utilization-target 1.0 \
  --no-optuna-enable \
  --no-optuna-warmstart-enable \
  --paper-split-protocol \
  --lgb-device-type cpu \
  --lgb-num-threads "${SLURM_CPUS_PER_TASK:-3}" \
  --lgb-num-leaves 255 \
