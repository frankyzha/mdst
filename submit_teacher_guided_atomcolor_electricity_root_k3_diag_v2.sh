#!/usr/bin/env bash
#SBATCH --job-name=msdt_tga_elec_rootk3diag_v2
#SBATCH --output=msdt_tga_elec_rootk3diag_v2_%j.out
#SBATCH --error=msdt_tga_elec_rootk3diag_v2_%j.err
...
def main() -> None:
    fn = getattr(_libgosdt, "msplit_debug_teacher_guided_atomcolor_root_feature", None)
    if fn is None:
        fn = getattr(_libgosdt, "msplit_debug_teacher_atomcolor_root_feature")
    ...
    for feature in [0]:
        ...
        log(f"feature {feature} debug done")
        payload = json.loads(raw)
        g3 = next(
            (entry for entry in payload.get("exact_bnb", []) if int(entry.get("groups", -1)) == 3),
            None,
        )
        diag = (g3 or {}).get("stage2_diag") or {}
        out[str(feature)] = {
            "feature_name": feature_names[feature] if feature < len(feature_names) else str(feature),
            "atoms": len(payload.get("atoms", [])),
            "stage2_state_calls": (g3 or {}).get("stage2_state_calls"),
            "max_seen_states": (g3 or {}).get("max_seen_states"),
            "best_leaf_cost": (g3 or {}).get("best_leaf_cost"),
            "coarse_bucket_count": diag.get("coarse_bucket_count"),
            "coarse_bucket_max_size": diag.get("coarse_bucket_max_size"),
            "support_bucket_count": diag.get("support_bucket_count"),
            "support_bucket_max_size": diag.get("support_bucket_max_size"),
            "coarse_top_buckets": diag.get("coarse_top_buckets", [])[:5],
            "support_top_buckets": diag.get("support_top_buckets", [])[:5],
        }
