...
def _stratified_split_indices(
    ...
    unique, inverse = np.unique(y_bin, return_inverse=True)
    class_counts = np.bincount(inverse, minlength=unique.size).astype(np.int64)
    sample_total = int(round(float(sample_size) * float(all_idx.size)))
    sample_total = max(0, min(sample_total, int(all_idx.size)))
    ...
    for class_id in range(unique.size):
        class_idx = all_idx[inverse == class_id].copy()
