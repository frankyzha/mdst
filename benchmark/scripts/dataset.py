...
from benchmark.scripts.benchmark_paths import BENCHMARK_DATA_ROOT, DEFAULT_OPENML_CACHE


DEFAULT_DATA_ROOT = BENCHMARK_DATA_ROOT
...
DATASET_SPECS: dict[str, DatasetSpec] = {
    ...
    "compas": DatasetSpec(
        ...
        local_path="benchmark/datasets/compas/raw/compas_data_matrix.csv",
    ...
    "coupon": DatasetSpec(
        ...
        task="classification",
        target_name="Y",
        local_path="benchmark/datasets/coupon/coupon_source.csv",
...
def materialize_dataset(
    ...
    *,
    ...
    force: bool = False,
    openml_cache_dir: str | Path | None = DEFAULT_OPENML_CACHE,
    build_cache_after_download: bool = True,
    cache_seed: int = 0,
    cache_test_size: float = 0.2,
    cache_val_size: float = 0.125,
    cache_max_bins: int = 1024,
    cache_min_samples_leaf: int = 8,
    cache_min_child_size: int = 8,
    lgb_num_threads: int = 6,
) -> tuple[Path, Path]:
    spec = get_dataset_spec(name)
    ...
    if build_cache_after_download:
        from benchmark.scripts.cache_utils import build_cache, default_cache_path

        cache_path = default_cache_path(
            dataset=spec.name,
            seed=cache_seed,
            test_size=cache_test_size,
            val_size=cache_val_size,
            max_bins=cache_max_bins,
            min_samples_leaf=cache_min_samples_leaf,
            min_child_size=cache_min_child_size,
        )
        build_cache(
            dataset=spec.name,
            seed=cache_seed,
        ...
        metadata["default_cache_path"] = str(cache_path)
    ...
    *,
    ...
    for dataset_name in dataset_names:
        ...
        csv_path, metadata_path = materialize_dataset(
            ...
            build_cache_after_download=build_cache_after_download,
            cache_seed=cache_seed,
            cache_test_size=cache_test_size,
            cache_val_size=cache_val_size,
            cache_max_bins=cache_max_bins,
            cache_min_samples_leaf=cache_min_samples_leaf,
            cache_min_child_size=cache_min_child_size,
            lgb_num_threads=lgb_num_threads,
        )
        realized[spec.name] = {
            "csv_path": str(csv_path),
...
def main() -> None:
    ...
    realized = materialize_all_datasets(
        ...
        build_cache_after_download=not bool(args.skip_cache),
        cache_seed=int(args.cache_seed),
        cache_test_size=float(args.cache_test_size),
        cache_val_size=float(args.cache_val_size),
        cache_max_bins=int(args.cache_max_bins),
        cache_min_samples_leaf=int(args.cache_min_samples_leaf),
