...
def _pack_ragged_float_arrays(arrays: List[np.ndarray], *, prefix: str) -> dict[str, np.ndarray]:
    flattened = [np.asarray(arr, dtype=np.float64).reshape(-1) for arr in arrays]
    lengths = np.asarray([arr.size for arr in flattened], dtype=np.int32)
    flat = np.concatenate(flattened, axis=0) if flattened else np.array([], dtype=np.float64)
    return {
...
def _unpack_ragged_float_arrays(lengths: np.ndarray, flat: np.ndarray) -> List[np.ndarray]:
    values = np.asarray(flat, dtype=np.float64).reshape(-1)
    out: List[np.ndarray] = []
    cursor = 0
    for length in np.asarray(lengths, dtype=np.int32).tolist():
        next_cursor = cursor + int(length)
        out.append(np.asarray(values[cursor:next_cursor], dtype=np.float64))
        cursor = next_cursor
    return out


def serialize_lightgbm_binner(binner: LightGBMBinner) -> dict[str, np.ndarray]:
    """Serialize the fitted LightGBM binner into ``np.savez``-friendly arrays."""

    payload = _pack_ragged_float_arrays(binner.bin_edges_per_feature, prefix="binner_bin_edges")
    payload["binner_fill_values"] = np.asarray(binner.fill_values_per_feature, dtype=np.float64)
    payload["binner_max_bins"] = np.asarray([int(binner.max_bins)], dtype=np.int32)
    payload["binner_min_samples_leaf"] = np.asarray([int(binner.min_samples_leaf)], dtype=np.int32)
    payload["binner_random_state"] = np.asarray(
        [-1 if binner.random_state is None else int(binner.random_state)],
        dtype=np.int32,
    )
    payload["binner_device_type"] = np.asarray([str(binner.device_type)], dtype=str)

    representatives = binner.bin_representatives_per_feature
    if representatives is not None:
        payload.update(_pack_ragged_float_arrays(representatives, prefix="binner_bin_representatives"))
    return payload


def deserialize_lightgbm_binner(arrays: Mapping[str, object]) -> LightGBMBinner:
    """Rebuild a ``LightGBMBinner`` from cached arrays when available."""

    bin_edges = _unpack_ragged_float_arrays(
        np.asarray(arrays["binner_bin_edges_lengths"], dtype=np.int32),
        np.asarray(arrays["binner_bin_edges_flat"], dtype=np.float64),
    )
    representatives = None
    if "binner_bin_representatives_lengths" in arrays and "binner_bin_representatives_flat" in arrays:
        representatives = _unpack_ragged_float_arrays(
            np.asarray(arrays["binner_bin_representatives_lengths"], dtype=np.int32),
            np.asarray(arrays["binner_bin_representatives_flat"], dtype=np.float64),
        )

    random_state = int(np.asarray(arrays["binner_random_state"], dtype=np.int32).reshape(-1)[0])
    if random_state < 0:
        random_state = None

    teacher_logit = None
    if "teacher_logit" in arrays:
        teacher_logit = np.asarray(arrays["teacher_logit"], dtype=np.float64)

    boundary_gain = None
    if "teacher_boundary_gain" in arrays:
        boundary_gain = np.asarray(arrays["teacher_boundary_gain"], dtype=np.float64)

    boundary_cover = None
    if "teacher_boundary_cover" in arrays:
        boundary_cover = np.asarray(arrays["teacher_boundary_cover"], dtype=np.float64)

    boundary_value_jump = None
    if "teacher_boundary_value_jump" in arrays:
        boundary_value_jump = np.asarray(arrays["teacher_boundary_value_jump"], dtype=np.float64)

    device_type = str(np.asarray(arrays["binner_device_type"]).reshape(-1)[0])
    return LightGBMBinner(
        bin_edges_per_feature=bin_edges,
        fill_values_per_feature=np.asarray(arrays["binner_fill_values"], dtype=np.float64),
        max_bins=int(np.asarray(arrays["binner_max_bins"], dtype=np.int32).reshape(-1)[0]),
        min_samples_leaf=int(np.asarray(arrays["binner_min_samples_leaf"], dtype=np.int32).reshape(-1)[0]),
        random_state=random_state,
        device_type=device_type,
        teacher_train_logit=teacher_logit,
        teacher_available=teacher_logit is not None,
        teacher_models=None,
        ...
        bin_representatives_per_feature=representatives,
    )
