...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to a CSV file. Defaults to benchmark dataset locations when available.",
    )
    parser.add_argument("--no_guess", action="store_true")
    args = parser.parse_args()

    # Read the dataset from the reorganized benchmark layout when available.
    if args.data_csv is not None:
        data_path = Path(args.data_csv).expanduser().resolve()
    else:
        project_root = Path(__file__).resolve().parents[4]
        candidates = [
            project_root / "benchmark" / "datasets" / args.dataset / "raw" / f"{args.dataset}_data_matrix.csv",
            project_root / "SPLIT-ICML" / "resplit" / "example" / f"{args.dataset}.csv",
            project_root / "datasets" / f"{args.dataset}.csv",
        ]
