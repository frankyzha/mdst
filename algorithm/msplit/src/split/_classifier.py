...
class GOSDTClassifier(ClassifierMixin, BaseEstimator):
    """
    Decision Tree Classifier that produces optimal sparse decision trees.
    ...
    regularization : float, default=0.05
        ...
        preferred, you must set the parameter `allow_small_reg` to True, which
        by default is False.
    ...
    def fit(self, X, y, y_ref=None, input_features=None, cost_matrix=None, feature_map=None):
        ...
        if not np.all(np.logical_or(X == 0, X == 1)):
            raise ValueError(
                "X must be boolean, but contains values other than 0 and 1."
            )

        # Check that regularization is large enough to be effective based on the number of samples:
        if self.regularization < 1.0 / n:
            print(
                f"[WARNING] A regularization was chosen that is less than 1 / (# of samples) = {1.0 / n}."
                " This may lead to a longer training time if not adjusted."
            )
            if not self.allow_small_reg:
                print(
                    f"[WARNING] Regularization increased to 1 / (# of samples) = {1.0 / n}."
                    f" If you would like to continue with your chosen regularization ({self.regularization}),"
                    " please set allow_small_reg=True."
                )
