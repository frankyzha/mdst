#!/bin/bash
#SBATCH --job-name=exp1_lb0p10m
#SBATCH --partition=compsci
#SBATCH --nodelist=compsci-cluster-fitz-36
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --mem=128G
#SBATCH --time=04:00:00
#SBATCH --output=results/runtime_compare_jobs/exp1_lb0prepass10m_depth1_no_intervalcheapub_fit36_%j.out
#SBATCH --error=results/runtime_compare_jobs/exp1_lb0prepass10m_depth1_no_intervalcheapub_fit36_%j.err
...
/usr/bin/time -v .venv/bin/python run_multisplit_experiments_lightgbm.py \
  --run-name exp1_lb0prepass10m_depth1_no_intervalcheapub_fit36_$(date +%Y%m%d_%H%M%S) \
  --datasets eye-state \
  --depth-budgets 4 \
  --seeds 0 \
  --time-limit 10800 \
