#!/bin/bash
#SBATCH --job-name=cmp_cached_msplit_newds
...
#SBATCH --output=results/runtime_compare_jobs/cmp_cached_msplit_newds_%j.out
#SBATCH --error=results/runtime_compare_jobs/cmp_cached_msplit_newds_%j.err
...
RUN_NAME="cached_msplit_linear_nonlinear_shapecart_adult_bike_sharing_$(date +%Y%m%d_%H%M%S)"
RESULTS_ROOT="results/comparison_cached_msplit_linear_nonlinear_shapecart_newdatasets"
