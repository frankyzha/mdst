#!/usr/bin/env bash
#SBATCH --job-name=msdt_boundarysig_d4
#SBATCH --output=msdt_boundarysig_d4_%j.out
#SBATCH --error=msdt_boundarysig_d4_%j.err
...
.venv/bin/python run_multisplit_experiments_lightgbm.py \
  --run-name benchmark_distilled_boundarysig4_electricity_d4_cluster_20260308 \
  ...
  --approx-distilled-geometry-mode teacher_hierarchy \
  --approx-score-order-enabled
