#!/usr/bin/env bash
#SBATCH --job-name=msdt_rc_prefix_opt_any_d4
#SBATCH --output=msdt_rc_prefix_opt_any_d4_%j.out
#SBATCH --error=msdt_rc_prefix_opt_any_d4_%j.err
...
.venv/bin/python run_multisplit_experiments_lightgbm.py \
  --run-name benchmark_distilled_runcolor_prefix_electricity_d4_opt_fitz_any_20260309 \
