                        ...
                        auto scan_component_overlap = [&](int lo, int hi) {
                            ...
                            for (int start = lo; start <= hi; ++start) {
                                ...
                                for (int end = start; end <= hi; ++end) {
                                    ...
                                    const double complexity_gain = -static_cast<double>(move.delta_components);
                                    if (std::abs(primary_gain) > kEpsUpdate || complexity_gain <= kEpsUpdate) {
                                        continue;
                                    }
                                    RankedMove ranked = make_ranked_move(
                                        std::move(move),
    ...
    AtomizedCandidate nominate_folded_family_candidate(
        ...
        if (!raw_seed.feasible || !prepared.has_block_compression || prepared.block_atoms.empty()) {
            return raw_seed;
        }
    ...
    GreedyResult greedy_complete(std::vector<int> indices, int depth_remaining) {
        ...
        if (depth_remaining <= 0) {
            ...
            cache_store(depth_remaining, std::move(indices), solved);
            return solved;
        }
        if (stats.pure) {
            GreedyResult solved{leaf_objective, leaf_tree};
        ...
        std::vector<double> feature_best_proxies((size_t)n_features_, kInfinity);
        for (int feature = 0; feature < n_features_; ++feature) {
            ...
            const auto &coarse_by_groups = prepared.coarse_by_groups;
            ...
            feature_best_proxies[(size_t)feature] = feature_best_proxy;
        }

        double best_coarse_proxy = kInfinity;
        for (double feature_best_proxy : feature_best_proxies) {
