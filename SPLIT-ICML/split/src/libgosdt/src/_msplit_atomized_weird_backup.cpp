    std::vector<AtomizedCandidate> select_atomized_candidates_for_arity(
        ...
        AtomizedCandidatePair raw_seed_pair;
        const AtomizedCandidate *impurity_raw_seed = nullptr;
        const AtomizedCandidate *misclassification_raw_seed = nullptr;
        if (prepared.has_block_compression) {
            raw_seed_pair = solve_atomized_geometry_family_pair(
                prepared.atoms,
                prepared.atom_prefix,
                feature,
                groups);
            impurity_raw_seed = &raw_seed_pair.impurity;
            misclassification_raw_seed = &raw_seed_pair.misclassification;
        }
    ...
    size_t collect_cheap_candidates(
        ...
        for (int feature = 0; feature < n_features_; ++feature) {
            ...
            for (size_t groups = 2; groups < prepared_ref.coarse_by_groups.size(); ++groups) {
                ...
                const bool hardloss_feasible = hardloss_coarse.candidate.feasible;
    ...
    GreedyResult greedy_complete_impl(
        ...
        const bool disable_pair_prune_globally = false;
        ...
        double best_candidate_upper = kInfinity;
        for (size_t idx = 0; idx < nominee_evals.size(); ++idx) {
            const double upper = nominee_evals[idx].upper_bound;
            if (upper < best_candidate_upper) {
                best_candidate_upper = upper;
            }
        }
        if (!std::isfinite(best_candidate_upper)) {
            GreedyResult solved{leaf_objective, leaf_tree};
            trace_greedy_snapshot(
                "return_no_alive_candidate",
                depth_remaining,
                indices.size(),
                preserved_feature_count,
                static_cast<unsigned long long>(surviving_candidate_evals.size()),
        ...
        if (disable_pair_prune_globally) {
            ...
            const double candidate_prune_cutoff = best_candidate_upper + prune_slack + kEpsUpdate;
            for (size_t idx = 0; idx < nominee_evals.size(); ++idx) {
                if (nominee_evals[idx].lower_bound <= candidate_prune_cutoff) {
                    alive_indices.push_back(idx);
                }
            }
        }
        if (alive_indices.empty()) {
            alive_indices.push_back((size_t)std::distance(
                nominee_evals.begin(),
                std::min_element(
                    nominee_evals.begin(),
                    nominee_evals.end(),
                    [](const NomineeEval &lhs, const NomineeEval &rhs) {
                        if (lhs.upper_bound != rhs.upper_bound) {
                            return lhs.upper_bound < rhs.upper_bound;
                        }
                        return lhs.lower_bound < rhs.lower_bound;
        ...
        std::priority_queue<std::pair<double, size_t>, std::vector<std::pair<double, size_t>>, std::greater<std::pair<double, size_t>>> lower_bound_heap;
        for (size_t idx = 0; idx < alive_indices.size(); ++idx) {
            const size_t candidate_idx = alive_indices[idx];
            lower_bound_heap.emplace(nominee_evals[candidate_idx].lower_bound, candidate_idx);
        }

        bool certificate_triggered = false;
        for (size_t order = 0; order < alive_indices.size(); ++order) {
            const size_t idx = alive_indices[order];
            if (finalized[idx]) {
                continue;
            }
            if (nominee_evals[idx].lower_bound > best_exact_objective + kEpsUpdate) {
                finalized[idx] = 1U;
                exact_results[idx] = ExactNomineeResult{};
                continue;
            }
            ...
            while (!lower_bound_heap.empty() && finalized[lower_bound_heap.top().second]) {
                lower_bound_heap.pop();
            }
            if (!lower_bound_heap.empty() &&
                best_exact_objective <= lower_bound_heap.top().first + mu_node + kEpsUpdate) {
        ...
        nominee_exactified_until_certificate_total_ += exactified_total;
        ...
        bump_count_histogram(
            ...
            static_cast<size_t>(exactified_total));
