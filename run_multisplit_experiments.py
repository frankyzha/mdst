"""Run CART-binned multi-split SPLIT experiments with VCM-friendly artifacts."""
...
from split import MSPLIT, fit_cart_binner
...
def _write_depth_log(
    ...
    with out_path.open("w", encoding="utf-8") as f:
        f.write("MSPLIT (CART bins + C++ DP/lookahead) depth vs. accuracy\n")
        f.write(
            "CONFIG: "
...
def main() -> None:
    ...
    with stdout_log_path.open("a", encoding="utf-8") as log_fp:
        ...
        _log("Starting multisplit experiment run.", log_fp)
        ...
        depth_log_path = run_dir / "multisplit_depth_vs_accuracy.log"
        plot_path = run_dir / "multisplit_cart_dp_accuracy.png"
        plot_vs_paper_path = run_dir / "multisplit_cart_dp_vs_paper_accuracy.png"
        ...
        # Write compatibility files with fixed names under results/.
        stable_csv = stable_results_dir / "multisplit_cart_dp_results.csv"
        stable_plot = stable_results_dir / "multisplit_cart_dp_accuracy.png"
        stable_log = stable_results_dir / "multisplit_depth_vs_accuracy.log"
        stable_plot_typo = stable_results_dir / "multisplit_cart_dpdp_accuracy.png"

        _safe_copy(summary_csv_path, stable_csv)
        _safe_copy(plot_path, stable_plot)
        _safe_copy(depth_log_path, stable_log)
        _safe_copy(plot_path, stable_plot_typo)
        ...
        artifact_paths = {
            ...
            "stable_plot_typo_png": stable_plot_typo,
        }
